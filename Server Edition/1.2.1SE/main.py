...
#   Version: 1.2.1SE (Server Edition)
...
DEFAULTSETTINGS = {"WEBHOOK_URL": "", "__version__" :  LOCALVERSION,  "merchant_detection" : False, "ping_glitched_id" : "everyone", "ping_dreamspace_id" : "everyone", "send_mari" : True, "ping_mari_id" : 0, "send_jester" : True, "ping_jester_id" : 0, "auto_craft_mode" : False, "auto_craft_item" : {"Potion of Bound" : False, "Heavenly Potion" : True, "Godly Potion (Zeus)" : True, "Godly Potion (Poseidon)" : True, "Godly Potion (Hades)" : True, "Warp Potion" : True, "Godlike Potion" : True},  "edit_settings_mode" : True, "failsafe_key" : "ctrl+e", "private_server_link" : "", "biomes" : {"snowy" : False, "windy" : False, "rainy" : False, "sand storm" : False, "hell" : False, "starfall" : False, "corruption" : False, "null" : False, "glitched" : True, "dreamspace" : True}, "send_start_message" : True, "aura_detection" : True, "minimum_aura_send" : "99998"}
VALIDSETTINGSKEYS = ["WEBHOOK_URL", "__version__", "merchant_detection", "ping_glitched_id", "ping_dreamspace_id", "send_mari", "ping_mari_id", "send_jester", "ping_jester_id", "auto_craft_mode", "auto_craft_item", "edit_settings_mode", "failsafe_key",  "private_server_link", "biomes", "send_start_message", "aura_detection", "minimum_aura_send"]
        ...
        try:
            if current_aura.lower() in auras.keys():
                ...
                if current_biome.lower() == auras[current_aura.lower()]["native_biome"].lower():
                    ...
                    emb.set_thumbnail(url=auras[current_aura.lower()]["img_url"])           
                if (int(auras[current_aura.lower()]["rarity"]) > int(settings["minimum_aura_send"])):
...
for _ in settings["biomes"].keys():
    if settings["biomes"][_]:
        ...
        break
if settings["aura_detection"]:
    aura_detection_thread = threading.Thread(target=aura_detection)
    aura_detection_thread.start()
