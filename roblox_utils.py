...
def _get_latest_log_content(purpose="generic"):
    ...
    logger.write_log(f"({purpose}) Attempting to access logs in directory: {logs_dir}")
    ...
    try:
        ...
        logger.write_log(f"({purpose}) Found latest log file: {latest_log_file}")

        temp_dir = tempfile.gettempdir()
        temp_file_path = os.path.join(temp_dir, f"solsrngbot_{purpose}_{os.path.basename(latest_log_file)}")

        logger.write_log(f"({purpose}) Attempting to copy log to temp file: {temp_file_path}")

        shutil.copy2(latest_log_file, temp_file_path)

        logger.write_log(f"({purpose}) Successfully copied log file. Reading temp file...")
