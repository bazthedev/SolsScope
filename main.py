...
if not os.path.exists("./settings.json"):
    ...
    x.write('{"timer" : 1800, "ssnorm" : true, "ssstor" : true, "ssinv" : true, "log_channel_id": 0}')
    x.close()
...
async def on_ready():
    ...
    print(f"Started at {now.strftime("%d/%m/%Y %H:%M:%S")}")
    if log_channel_id != 0:
        ...
        print("You must select a channel ID, you can do this by running the set_log_channel command.") # It is recommended that you restart the bot after using this command, however it will still send messages to the new specified channel
